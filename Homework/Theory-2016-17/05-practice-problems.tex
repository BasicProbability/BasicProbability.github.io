...
\begin{exercise}[Maximum a posteriori estimates]
	...
	\[
		P(\Theta = \theta) = \frac{1}{B(\alpha, \beta)} \theta^{\alpha} (1-\theta)^{\beta},
	\] 
	...
	\begin{mycomment}
		Is this correctly parametrized? Normally you use $\alpha, \beta>0$ and $\alpha-1, \beta-1$ in the exponent, right?
	\end{mycomment}
	...
	\begin{subex}
		...
		\[
			\mathcal P(\theta) = \ln \bigl[ P(X_1, \dots, X_N \mid \Theta=\theta) \cdot P(\Theta = \theta)\bigr]
		\]
	\end{subex}
