...
= \Sexpr{geomLik}
\end{align*}
\item $ P(Y = c_{1}|X=\Sexpr{data[1]}, \Theta=\theta^{(0)}) = \dfrac{\Sexpr{geomweights[1]}\cdot \Sexpr{geomparams[1]}\cdot \Sexpr{1-geomparams[1]}^{\Sexpr{data[1]}}}{\Sexpr{geomweights[1]}\cdot \Sexpr{geomparams[1]}\cdot \Sexpr{1-geomparams[1]}^{\Sexpr{data[1]}} + \Sexpr{geomweights[2]} \cdot \Sexpr{geomparams[2]} \cdot \Sexpr{1-geomparams[2]}^{\Sexpr{data[1]}}} = \Sexpr{posteriors[1]} $
\\ \vspace{.4cm}
$ P(Y = c_{1}|X=\Sexpr{data[2]}, \Theta=\theta^{(0)}) = \dfrac{\Sexpr{geomweights[1]}\cdot \Sexpr{geomparams[1]}\cdot \Sexpr{1-geomparams[1]}^{\Sexpr{data[2]}}}{\Sexpr{geomweights[1]}\cdot \Sexpr{geomparams[1]}\cdot \Sexpr{1-geomparams[1]}^{\Sexpr{data[2]}} + \Sexpr{geomweights[2]} \cdot \Sexpr{geomparams[2]} \cdot \Sexpr{1-geomparams[2]}^{\Sexpr{data[2]}}} = \Sexpr{posteriors[2]} $
\\ \vspace{.4cm}
$ P(Y = c_{1}|X=\Sexpr{data[4]}, \Theta=\theta^{(0)}) = \dfrac{\Sexpr{geomweights[1]}\cdot \Sexpr{geomparams[1]}\cdot \Sexpr{1-geomparams[1]}^{\Sexpr{data[4]}}}{\Sexpr{geomweights[1]}\cdot \Sexpr{geomparams[1]}\cdot \Sexpr{1-geomparams[1]}^{\Sexpr{data[4]}} + \Sexpr{geomweights[2]} \cdot \Sexpr{geomparams[2]} \cdot \Sexpr{1-geomparams[2]}^{\Sexpr{data[4]}}} = \Sexpr{posteriors[4]} $
\end{enumerate}
\end{frame}
...
&\mathbb{E}[\sum_{i}\id{Y_{i}=c_{2}}\mid X=x, \Theta=\theta^{(0)}] = \Sexpr{datasize} - \Sexpr{sum(posteriors)} = \Sexpr{datasize-sum(posteriors)} \\
&\mathbb{E}[\sum_{i}x_{i}\id{Y_{i}=c_{1}}\mid X=x, \Theta=\theta^{(0)}] = \Sexpr{posteriors[1]} \cdot \Sexpr{data[1]} + 
2\cdot \Sexpr{posteriors[2]} \cdot \Sexpr{data[2]} + \Sexpr{posteriors[4]} \cdot \Sexpr{data[4]}\\
&= \Sexpr{expectedSum} \\
&\mathbb{E}[\sum_{i}x_{i}\id{Y_{i}=c_{2}}\mid X=x, \Theta=\theta^{(0)}] = \Sexpr{1-posteriors[1]} \cdot \Sexpr{data[1]} + 
...
&P(Y=c_{2}\mid \Theta = \theta^{(1)}) = 1 - \Sexpr{sum(posteriors) / (datasize - sum(posteriors))}
= \Sexpr{new_weights[2]} 
\end{align*}
new parameter of $ c_{1} $ : $ \dfrac{\Sexpr{sum(posteriors)}}{\Sexpr{expectedSum} + \Sexpr{sum(posteriors)}} = \Sexpr{new_params[1]} $\vspace{.5cm}

new parameter of $ c_{2} $ : $ \dfrac{\Sexpr{datasize - sum(posteriors)}}{\Sexpr{sum(data) - expectedSum} + \Sexpr{datasize - sum(posteriors)}} = \Sexpr{new_params[2]} $
\end{frame}
